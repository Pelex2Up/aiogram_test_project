...
from aiogram_timepicker.panel.single import minute, hour
from . import _default, timepicker_callback, adapter


class TimePicker:
    def __init__(self, interval: int = 30, callback: CallbackData = timepicker_callback, **kwargs):
        self.cancel = None
        ...
        for _td_i in range(len(_tds)):
            ...
            inline_kb.insert(InlineKeyboardButton(
                ...
                callback_data=self.callback.new(
                    "CHANGE", t.strftime('%H'), t.strftime("%M")),
            ))
        inline_kb.row()
    ...
    async def start_picker(
        ...
        ignore_callback = self.callback.new("IGNORE", hour, minute)
        _time = datetime(1970, 1, 1, hour, minute)
        if self.is_atomic:
        ...
        inline_kb.insert(InlineKeyboardButton(
            ...
            callback_data=self.callback.new('CHOOSE_H', hour, minute),
        ))
        inline_kb.insert(InlineKeyboardButton(
            self.minute_format.format(minute % 60),
            callback_data=self.callback.new('CHOOSE_M', hour, minute),
        ))
        # inline_kb.insert(InlineKeyboardButton(
        ...
        inline_kb.insert(InlineKeyboardButton(
            self.label_select,
            callback_data=self.callback.new('SELECTED', hour, minute),
        ))
        return inline_kb

    async def _process_selection_all_int(self, query: CallbackQuery, act: str, minutes: int, hours: int):
        if act == "CHANGE":
            await query.message.edit_reply_markup(
                await self.start_picker(hours, minutes, cancel=self.cancel)
            )
            ...
            kb = await hour.TimePicker(
                1, None,
                str_callback=self.callback.new('CHANGE', '{hour}', minutes))\
                .start_picker(hours)
    ...
    async def _process_selection_not_int(self, query: CallbackQuery, act: str, minutes: str, hours: str):
    ...
    async def process_selection(self, query: CallbackQuery, data: CallbackData) -> result.Result:
        ...
        if data['act'] == "CANCEL":
            ...
            return result.Result()
        if not (data['hour'].isdigit() and data['minute'].isdigit()):
            return await self._process_selection_not_int(
                query, data['act'], data['minute'], data['hour']
            )
        return await self._process_selection_all_int(
            query, data['act'], int(data['minute']), int(data['hour'])
        )
